...
var Shared = /** @class */ (function () {
    function Shared($, window, document) {
        var _this = this;
        this.$ = $;
        this.window = window;
        this.document = document;
        this.didScroll = false;
        this.changeHeaderOnScrollPixels = 220;
        this.init = function () {
            _this.initHeaderAnimation();
            _this.initMenuScrollSpy();
            _this.initPageScrollSmooth();
        };
        this.initPageScrollSmooth = function () {
            _this.$("a.page-scroll").bind("click", function (event) {
                var anchor = _this.$(event.target);
                _this.$('html, body').stop().animate({
                    scrollTop: _this.$(anchor.attr('href')).offset().top
                }, 1500, 'easeInOutExpo');
                event.preventDefault();
            });
        };
        this.initMenuScrollSpy = function () {
            _this.$("body").scrollspy({
                target: '.navbar-fixed-top'
            });
        };
        this.initHeaderAnimation = function () {
            _this.scroll();
            _this.$(_this.window).on("scroll", function (e) {
                if (!_this.didScroll) {
                    _this.didScroll = true;
                    setTimeout(_this.scroll, 250);
                }
            });
        };
        this.scroll = function () {
            var currentOffset = _this.window.pageYOffset || _this.document.scrollTop;
            if (currentOffset >= _this.changeHeaderOnScrollPixels) {
                _this.header.addClass('navbar-shrink');
            }
            else {
                _this.header.removeClass('navbar-shrink');
            }
